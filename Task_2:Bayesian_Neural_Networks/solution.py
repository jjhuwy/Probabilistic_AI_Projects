        ...
        for _ in progress_bar:
            ...
            for batch_idx, (batch_x, batch_y) in enumerate(train_loader):
                ...
                if isinstance(self.network, DenseNet):
                    ...
                    # forward propagate through the model to obtain predictions
                    y_pred, log_p, log_v = self.network(batch_x)
                    
                    # calculate loss
                    nll_ = F.nll_loss(F.log_softmax(y_pred, dim=1), batch_y, reduction='sum')
                    loss = 1/num_batches * (log_v - log_p) + nll_
...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        self.prior_sigma = torch.tensor(1.0)
        self.prior = UnivariateGaussian(self.prior_mu, self.prior_sigma)
        ...
        #  )
        self.weight_mu = torch.nn.Parameter(torch.Tensor(self.out_features, self.in_features).uniform_(-0.2, 0.2))
        self.weight_rho = torch.nn.Parameter(torch.Tensor(self.out_features, self.in_features).uniform_(-5, -4))
        ...
        if self.use_bias:
            ...
            self.bias_prior = UnivariateGaussian(self.prior_mu, self.prior_sigma)           
            self.bias_mu = torch.nn.Parameter(torch.Tensor(out_features).uniform_(-0.2, 0.2))
            self.bias_sigma = torch.nn.Parameter(torch.Tensor(out_features).uniform_(-5, -4))
            self.bias_var_posterior = MultivariateDiagonalGaussian(self.bias_mu, self.bias_sigma)
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        if self.training:
            self.log_prior = self.prior.log_likelihood(weights) + self.bias_prior.log_likelihood(bias)
...
class BayesNet(nn.Module):
    ...
    def __init__(self, in_features: int, hidden_features: typing.Tuple[int, ...], out_features: int):
        ...
        for i in range(num_affine_maps):
            self.layers.append(BayesianLayer(feature_sizes[i], feature_sizes[i + 1], bias=True))
    ...
    def log_prior(self):
        ...
        for layer in self.layers:
            lp = lp + layer.log_prior
        return lp
    
    def log_var_posterior(self):
        lvp = 0
        for layer in self.layers:
            lvp = lvp + layer.log_variational_posterior
        return lvp
    ...
    def forward(self, x: torch.Tensor) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for idx, current_layer in enumerate(self.layers):
            new_features = current_layer(current_features)[0]
        ...
        log_variational_posterior= self.log_var_posterior()
...
class UnivariateGaussian(ParameterDistribution):
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        # TODO: Implement this
        prob = torch.log(torch.exp(self.gaussian.log_prob(values))).sum()
        return prob

    def sample(self) -> torch.Tensor:
        # TODO: Implement this
        samp = torch.normal(self.mu, self.sigma)
        return samp
...
class MultivariateDiagonalGaussian(ParameterDistribution):
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        # TODO: Implement this
        prob = (-math.log(math.sqrt(2 * math.pi)) 
