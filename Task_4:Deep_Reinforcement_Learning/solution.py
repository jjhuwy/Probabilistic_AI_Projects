...
class VPGBuffer:
    ...
    def end_traj(self, last_val=0):
        ...
        deltas_phi = rews[:-1] + vals[1:] - vals[:-1]
        self.phi_buf[path_slice] = discount_cumsum(deltas_phi, self.gamma*self.lam)
...
class Agent:
    def __init__(self, env):
        ...
        self.ac = MLPActorCritic(hidden_sizes=[self.hid, self.hid*2, self.hid])
    ...
    def v_update(self, data):
        ...
        for _ in range(100):
            ...
            v = self.ac.v.forward(obs)
            loss_function = MSELoss()
            v_loss = loss_function(v, ret)
            v_loss.backward()
    ...
    def train(self):
        ...
        for epoch in range(epochs):
            ...
            for t in range(steps_per_epoch):
                a, v, logp = self.ac.step(torch.as_tensor(state, dtype=torch.float32, device=device))
                ...
                if terminal or timeout or epoch_ended:
                    ...
                    if epoch_ended:
                        _, v, _ = self.ac.step(torch.as_tensor(state, dtype=torch.float32, device=device))
                    else:
                        v = 0
    ...
    def get_action(self, obs):
        ...
        # MY CODE
        action = self.ac.pi._distribution(torch.as_tensor(obs, dtype=torch.float32, device=device))
